...
//
    ...
    if ($category_ids) {
        ...
        $objects = array_values(array_map(function ($category) use ($view, $company_id) {
            if (empty($_REQUEST['bulk_edit_mode'])) {
                ...
                return array(
                    ...
                    'data' => array(
                        'disabled' => $company_id && !empty($category['company_id']) && $company_id != $category['company_id'],
                        'content' => $view->fetch('views/categories/components/categories_bulk_edit_select2_item.tpl'),
                        'remover' => false,
                        'status' => $_REQUEST['map'][$category['category_id']]['status']
                    )
                );
            }
        }, $categories_data));
    ...
    if (!empty($_REQUEST['bulk_edit_mode']) || $action === 'bulk_edit') {
        $objects = array_filter($objects, function ($i) {
            return $i['data']['disabled'] === false;
        });
    }
    ...
    if ($action === 'bulk_edit') {
        ...
        foreach ($objects as $object) {
            if (isset($_REQUEST['categories_products'][$object['id']])
                && isset($_REQUEST['product_ids'])
                && count($_REQUEST['product_ids']) === count($_REQUEST['categories_products'][$object['id']])
            ) {
                $bulk_edit_ids[$object['id']]['status'] = 'A';
            } else {
                $bulk_edit_ids[$object['id']]['status'] = 'N';
            }
        }

        Tygh::$app['view']->assign([
            'bulk_edit_ids_flat' => array_keys($bulk_edit_ids),
            'bulk_edit_ids'      => $bulk_edit_ids,
            'bulk_edit'          => true
        ]);
        Tygh::$app['view']->display('views/products/components/bulk_edit/categories.tpl');
    } else {
