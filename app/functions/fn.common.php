 ...
 * @param bool   $clear_cache If set to true all cache and cacheHandlers will be removed
 *
 * @return boolean always true
 */
function fn_set_store_mode($store_mode, $company_id = null, $clear_cache = true)
{
    ...
    $result = true;
    $allowed_modes = array('opened', 'closed');

    if (in_array($store_mode, $allowed_modes)) {
        $mode = $store_mode == 'opened' ? StorefrontStatuses::OPENED : StorefrontStatuses::CLOSED;
        /** @var Tygh\Settings $settings */
        $settings = Settings::instance();

        if ($settings->getValue('store_mode', 'General', $company_id) != $mode) {
            $settings->updateValue('store_mode', $mode, 'General', $clear_cache, $company_id);

            $result = $settings->getValue('store_mode', 'General', $company_id) == $mode;
