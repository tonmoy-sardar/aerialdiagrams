...
/**
 * Calculates storefronts statistics that disabled and saves to runtime storage
 *
 * @param array|null $available_company_ids Company ids
 *
 * @return void
 * @internal
 */
function fn_ult_init_storefronts_stats($available_company_ids = null)
{
    $closed_storefronts_qty = 0;
    $available_company_ids = $available_company_ids ?: fn_get_available_company_ids();

    foreach ($available_company_ids as $id) {
        $status = fn_ult_get_storefront_status($id);

        if ($status === StorefrontStatuses::CLOSED) {
            $closed_storefronts_qty++;
        }
    }

    Registry::set('runtime.are_all_storefronts_closed', $closed_storefronts_qty === count($available_company_ids));
    Registry::set('runtime.have_closed_storefronts', $closed_storefronts_qty > 0);
}
...
{
    if (AREA == 'A') {
        fn_ult_init_storefronts_stats($available_company_ids);
    }
...
{
    return Settings::instance()->getValue('store_access_key', 'General', $company_id);
}
...
{
    return Settings::instance()->getValue('store_mode', 'General', $company_id);
}
...
{
    return fn_set_store_mode('opened', $company_id, false);
}
...
{
    $is_store_closed = fn_set_store_mode('closed', $company_id, false);

    if ($is_store_closed) {
        $current_access_key = fn_ult_get_storefront_access_key($company_id);

        if ($current_access_key === '') {
            $new_access_key = md5(TIME);
            Settings::instance()->updateValue('store_access_key', $new_access_key, 'General', false, $company_id);
        }
    }
...
{
    foreach ($companies as &$company) {
        if (!empty($params['extend']['storefront_status'])) {
            $company['storefront_status'] = fn_ult_get_storefront_status($company['company_id']);
        }

        if (!empty($params['extend']['storefront_access_key'])) {
            $company['store_access_key'] = fn_ult_get_storefront_access_key($company['company_id']);
        }
    }
    unset($company);
}
...
{
    if (!empty($extra['get_storefront_status'])) {
        $company_data['storefront_status'] = fn_ult_get_storefront_status($company_data['company_id']);
    }

    if (!empty($extra['get_access_key'])) {
        $company_data['store_access_key'] = fn_ult_get_storefront_access_key($company_data['company_id']);
    }
}