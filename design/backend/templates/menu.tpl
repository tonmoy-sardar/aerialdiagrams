...
{if "MULTIVENDOR"|fn_allowed_for}
    {$is_current_storefront_closed = $settings.General.store_mode === "StorefrontStatuses::CLOSED"|enum}
{else}
    {$is_current_storefront_closed = $runtime.company_data.company_id && $settings.General.store_mode === "StorefrontStatuses::CLOSED"|enum}
{/if}

{if $is_current_storefront_closed}
    {$access_key = $settings.General.store_access_key}
    {$store_access_key = "store_access_key=`$access_key`"}
{/if}
...
<div class="navbar-admin-top cm-sticky-scroll" data-ca-stick-on-screens="sm-large,md,md-large,lg,uhd" data-ca-top="{$sticky_top}" data-ca-padding="{$sticky_padding}">
    ...
    <div class="navbar navbar-inverse mobile-hidden" id="header_navbar">
        <div class="navbar-inner{if $is_current_storefront_closed || $runtime.are_all_storefronts_closed} navbar-inner--disabled{/if}">
        ...
        {if "ULTIMATE"|fn_allowed_for}
            {if $is_current_storefront_closed || $runtime.are_all_storefronts_closed}
            ...
            <div class="nav-ult">
                ...
                {if $runtime.company_data.storefront}
                    <a href="{"?company_id=`$runtime.company_data.company_id`"|fn_link_attach:$store_access_key|fn_url:"C"}" target="_blank" class="brand" title="{__("view_storefront")}">
        ...
        {if "MULTIVENDOR"|fn_allowed_for && !$runtime.simple_ultimate}
            ...
            {if $is_current_storefront_closed}
                {$storefront_status_icon = "icon-lock"}
            {else}
                {$storefront_status_icon = false}
            {/if}

            <ul class="nav">
                <li class="nav-company">
                    {$storefront_url = $config.http_location|fn_url}
                    {if $store_access_key}
                        {$storefront_url = $storefront_url|fn_link_attach:$store_access_key}
                    {/if}
                    <a href="{$storefront_url|escape}" target="_blank" class="brand" title="{__("view_storefront")}">
                        <i class="icon-shopping-cart icon-white"></i>
                    </a>
                    <a href="{""|fn_url}" class="brand company-name">{$settings.Company.company_name|truncate:60:"...":true}</a>
                    {if $storefront_status_icon}
                    <a href="{"settings.manage?section_id=General&highlight=store_mode,store_access_key"|fn_url}" class="brand">
    ...
    <div class="subnav" id="header_subnav">
        ...
        <ul class="nav hover-show nav-pills">
                ...
                {if "MULTIVENDOR"|fn_allowed_for && !$runtime.simple_ultimate}
                    ...
                    <p class="menu-heading__title-block mve">
                        <span class="menu-heading__title-block--text">
                            ...
                            <a href="{"settings.manage?section_id=General&highlight=store_mode,store_access_key"|fn_url}">
            ...
            {if $runtime.company_data.storefront}
                <li class="dropdown">
                    <a  href="{"?company_id=`$runtime.company_data.company_id`"|fn_url:"C"|fn_link_attach:$store_access_key}"
                        target="_blank"
            ...
            {else}
                {if "MULTIVENDOR"|fn_allowed_for}
                    <li class="dropdown">
                        <a  href="{""|fn_url:"C"|fn_link_attach:$store_access_key}"
                            target="_blank"
                            title="{__("view_storefront")}"
                            class="dropdown-toggle">{__("view_storefront")}</a>
                    </li>
